...
static void
dump_objset(objset_t *os, int compress_alg_index)
{
...
dump_one_objset(const char *dsname, void *arg)
{
	...
	dump_objset(os, compress_alg_index);
	close_objset(os, FTAG);
	fuid_table_destroy();
	return (0);
}
...
dump_zpool(spa_t *spa, int compress_alg_index)
{
	...
	if (dump_opt['d'] || dump_opt['i']) {
		...
		mos_refd_objs = range_tree_create(NULL, RANGE_SEG64, NULL, 0,
		    0);
		dump_objset(dp->dp_meta_objset, compress_alg_index);
        ...
        int compress_alg_index)
{
    ...
    //  for (lsize = 13312; lsize <= 16777216; lsize += 512)
	if (bp_lsize != -1)
		psize = bp_lsize;  // 12800 0x03200
	for (lsize = psize + SPA_MINBLOCKSIZE;
	    lsize <= SPA_MAXBLOCKSIZE; lsize += SPA_MINBLOCKSIZE) {
		//if (bp_lsize != -1)
		//	lsize = bp_lsize;  // 12800 0x03200
...
main(int argc, char **argv)
{
	...
	if (!dump_opt['R']) {
		...
		if (os != NULL) {
			...
			dump_objset(spa->spa_meta_objset, compress_alg_index);
		} else {
